...
require_relative './models/online'
require_relative './models/online_file'
require_relative './models/mail'
require_relative './models/mailbox'
require_relative './models/mail_history'
require_relative './models/code'
# Concerns: reusable modules
require_relative './controller_concerns/permission_authable'
# File transfer
require 'uri'
require 'fileutils'
...
namespace '/files' do
  ...
  get '/download/:sender/:file' do
    ...
    path = 
      Path::FILE_PATH + 
      "/#{user.username}/#{params[:sender]}/#{params[:file]}"
    puts path
...
namespace '/codes' do
  ...
  get '/new' do    
    @lang_list = 
      Rouge::Lexers.constants
      .select {|c| Rouge::Lexers.const_get(c).is_a? Class}
      .map{|c| c.to_s}
    erb :'codes/new'
  end
